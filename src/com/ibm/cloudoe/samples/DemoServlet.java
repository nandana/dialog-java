...
public class DemoServlet extends HttpServlet {
	...
	protected void doPost(final HttpServletRequest req,
		...
		try {
			URI converseURI = new URI(baseURL + "/v1/dialogs/" + dialogId
					+ "/conversation").normalize();

			Request converseRequest = Request.Post(converseURI).bodyForm(
					Form.form().add("input", input).add("client_id", clientId)
					.add("conversation_id", conversationId).build());

			Executor executor = Executor.newInstance().auth(username, password);
			Response response = executor.execute(converseRequest);
